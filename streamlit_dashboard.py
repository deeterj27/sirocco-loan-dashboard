...
def process_life_settlement_data(ls_file):
    ...
    try:
        ...
        for row in range(3, 200):
            ...
            try:
                # Get NDB value first
                ndb_value = safe_float(str(val_sheet[f'V{row}'].value or '0').replace('
...
""", unsafe_allow_html=True), '').replace(',', ''))
                ...
                if ndb_value == 0:
                    ...
                    face_amount = safe_float(str(val_sheet[f'W{row}'].value or '0').replace('
...
""", unsafe_allow_html=True), '').replace(',', ''))
                    if face_amount == 0:
                        ...
                        for col in ['X', 'Y', 'U', 'T']:
                            face_amount = safe_float(str(val_sheet[f'{col}{row}'].value or '0').replace('
...
""", unsafe_allow_html=True), '').replace(',', ''))
                ...
                policy_data = {
                    ...
                    'NDB': ndb_value,
                    'Valuation': safe_float(str(val_sheet[f'Z{row}'].value or '0').replace('
...
""", unsafe_allow_html=True), '').replace(',', '')),
                    'Cost_Basis': safe_float(str(val_sheet[f'AB{row}'].value or '0').replace('
